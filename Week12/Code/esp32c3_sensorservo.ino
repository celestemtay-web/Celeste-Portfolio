...
void loop() {
  ...
  if (distance_cm < DISTANCE_THRESHOLD) {
    ...
    digitalWrite(8, LOW);  // change state of the LED by setting the pin to the HIGH voltage level
  } else {
    servo.write(0); // *** edit the rotation of the servo here (within range: 0 – 180)
    digitalWrite(8, HIGH);   // change state of the LED by setting the pin to the LOW voltage level                   // wait for a second
  } 
  // ====== Serial Output ======
