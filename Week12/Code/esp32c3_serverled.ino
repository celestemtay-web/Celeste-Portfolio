      ...
      if (client.available()) {             // if there's bytes to read from the client
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            client.println("<style>html { font-family: Luminari; display: inline-block; margin: 0px auto; text-align: center;}");
            client.println(".button { background-color: #aed4a5; border: none; color: white; padding: 20px 150px;");
            client.println("text-decoration: none; font-size: 30px; margin: 2px; cursor: pointer;}");
            client.println(".button2 {background-color: #ffb0c5;}</style></head>");
            
            // ====== Webpage Title ======
            // *** edit text here
            client.println("<body><h1>ESP32 Celeste Button</h1>");
